...
def proccess_videos(videos_url, name_cam):
    ...
    while True:
        while video_capture.isOpened():
            ...
            if not ret:
                break
            results = model.track(source=frame, conf=0.5, classes=0, show=True)
            for i in results[0]:
                boxes = i.boxes.numpy()
                ...
                if x1 and x2 and y1 and y2:
                    ...
                    if flag2 == False and not os.path.isdir("detected_from_video_screen/{0}".format(name_cam)):
                        ...
                        flag2 = True
                    if not os.path.isdir('detected_from_video_screen/{0}/id{1}'.format(name_cam, count)):
                        os.mkdir('detected_from_video_screen/{0}/id{1}'.format(name_cam, count))
                    if not os.path.isdir("blur-face/{0}/id{1}".format(name_cam, count)):
                        os.mkdir("blur-face/{0}/id{1}".format(name_cam, count))
